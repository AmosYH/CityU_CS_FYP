...
def main():
    ...
    # train_low_data_names = './data/DarkFace_Train_2021/image'
    train_low_data_names = './data/combine/train_images'
    TrainDataset = MemoryFriendlyLoader(img_dir=train_low_data_names, task='train')


    # test_low_data_names = './data/VELOL/VE-LOL-L-Cap-Low_test'
    test_low_data_names = './data/difficult'
    TestDataset = MemoryFriendlyLoader(img_dir=test_low_data_names, task='test')

    train_queue = torch.utils.data.DataLoader(
        TrainDataset, batch_size=args.batch_size, generator=torch.Generator(device='cuda'),
        pin_memory=True, num_workers=0, shuffle=True)

    test_queue = torch.utils.data.DataLoader(
        TestDataset, batch_size=1, generator=torch.Generator(device='cuda'),
        pin_memory=True, num_workers=0, shuffle=True)
